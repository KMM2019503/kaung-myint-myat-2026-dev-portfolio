...
export function PersonalProjectCard({ project, projectNumber }: PersonalProjectCardProps) {
	const [isDetailsOpen, setIsDetailsOpen] = useState(false);
	const drawerBodyRef = useRef<HTMLDivElement>(null);
	const previewTechStack = project.techStack.slice(0, 4);

	useEffect(() => {
		if (!isDetailsOpen || !drawerBodyRef.current) {
			return;
		}
				...
				<Flex position="relative" direction="column" h="full" gap={{ base: 3.5, md: 4 }}>
					<Flex direction="column" gap={{ base: 4.5, md: 5 }}>
						...
						<Box
							...
							border="1px solid color-mix(in srgb, var(--surface-floating-border) 94%, transparent)"
							boxShadow="0 20px 32px -26px rgba(13, 21, 33, 0.72)"
						>
							<Image
